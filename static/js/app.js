  ...
  document.querySelectorAll(".nav-link").forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      if (targetId === "#home") {
        ...
        document
          .querySelector(".trends-grid")
          .scrollIntoView({ behavior: "smooth" });
      } else if (targetId === "#insights") {
        document
          .querySelector(".insights-section")
          .scrollIntoView({ behavior: "smooth" });
...
function renderTrends() {
  const filteredTrends =
    state.selectedPlatform === "all"
      ? state.trends
      : state.trends.filter((trend) =>
          trend.sources?.includes(state.selectedPlatform)
        );
  ...
  elements.trendsGrid.innerHTML = filteredTrends
    .map(
      (trend, index) => `
        <div class="trend-card" data-trend='${JSON.stringify(trend).replace(
          /'/g,
          "&#39;"
        )}'>
            ...
            <div class="trend-platforms">
                ${(trend.sources || [])
                  .map(
                    (source) => `
                    <span class="platform-badge">${getPlatformName(
                      source
                    )}</span>
                `
                  )
            ...
            <div class="trend-actions">
                <button class="action-btn primary" onclick="event.stopPropagation(); openOriginalLinks('${
                  trend.keyword
                }')">
                ...
                <button class="action-btn secondary" onclick="event.stopPropagation(); showTrendDetail('${
                  trend.keyword
                }')">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
...
function renderInsights() {
  ...
  elements.insightsGrid.innerHTML = state.insights
    .map(
      (insight, index) => `
            ...
            <div class="insight-hooks">
                ${(insight.hook_copies || [])
                  .map(
                    ...
                    <div class="hook-item">
                        ${hook}
                        <button class="copy-btn" onclick="copyToClipboard('${hook.replace(
                          /'/g,
                          "\\'"
                        )}')">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
...
async function showTrendDetail(keyword) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/keywords/details/${encodeURIComponent(keyword)}`
    );
    ...
    let detailHtml = `
            ...
            <div class="modal-stats">
                ...
                <div class="stat">
                    <span>관련 링크</span>
                    <strong>${
                      data.success ? data.data.urls.length : 0
                    }개</strong>
                </div>
                <div class="stat">
                    <span>총 점수</span>
                    <strong>${
                      data.success ? data.data.total_score : trend.score
                    }</strong>
                </div>
            </div>
            <div class="modal-platforms">
                ${(trend.sources || [])
                  .map(
                    (source) => `
                    <span class="platform-badge large">${getPlatformName(
                      source
                    )}</span>
                `
                  )
                        ...
                        ${data.data.urls
                          ...
                          .map(
                            ...
                            <div class="link-preview" onclick="window.open('${url}', '_blank')">
                                ...
                                <div class="link-text">
                                    <div class="link-domain">${getDomainFromUrl(
                                      url
                                    )}</div>
                                    <div class="link-url-short">${url.substring(
                                      0,
                                      50
                                    )}${url.length > 50 ? "..." : ""}</div>
                                </div>
      ...
      detailHtml += `
                <div class="modal-metadata">
                    ...
                    <div class="metadata-grid">
                        ${Object.entries(data.data.metadata)
                          .map(
                            ([key, value]) => `
                            <div class="metadata-item">
                                <span class="metadata-key">${getMetadataLabel(
                                  key
                                )}</span>
                                <span class="metadata-value">${formatMetadataValue(
                                  key,
                                  value
                                )}</span>
                            </div>
    ...
    detailHtml += `
            ...
            <div class="modal-actions" style="display: flex; gap: 10px; margin-top: 20px;">
                ...
                <button class="secondary-btn" onclick="saveTrend(${JSON.stringify(
                  trend
                ).replace(/"/g, "&quot;")})">
...
async function fetchKeywordHistory(keyword) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/keywords/history/${encodeURIComponent(keyword)}`
    );
    const data = await response.json();
...
function showSavedItems() {
  try {
    ...
    // Render saved trends
    elements.trendsGrid.innerHTML = savedItems
      .map(
        (trend, index) => `
            <div class="trend-card saved-item" onclick="showTrendDetail('${
              trend.keyword
            }')">
                <div class="card-header">
                    <div class="trend-rank">#${index + 1}</div>
                    <button class="bookmark-btn active" onclick="event.stopPropagation(); removeSavedItem('${
                      trend.id
                    }')">
                ...
                <div class="saved-date" style="margin-top: 8px; font-size: 0.875rem; color: var(--text-secondary);">
                    저장일: ${new Date(trend.savedAt).toLocaleDateString(
                      "ko-KR"
                    )}
                </div>
            </div>
...
async function openOriginalLinks(keyword) {
  try {
    ...
    if (data.success && data.data.urls && data.data.urls.length > 0) {
      ...
      showToast(
        `${getPlatformName(trend.sources[0])}에서 검색결과를 열었습니다`,
        "info"
      );
    }
  } catch (error) {
                    ...
                    ${urls
                      .map(
                        ...
                        <div class="link-item" onclick="openSingleLink('${url}')">
                            ...
                            <div class="link-info">
                                <div class="link-title">${getDomainFromUrl(
                                  url
                                )}</div>
                                <div class="link-url">${url.substring(0, 60)}${
                          url.length > 60 ? "..." : ""
                        }</div>
                            </div>
                        </div>
